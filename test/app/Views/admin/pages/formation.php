...
<?php echo @view('admin/_partials/sidebar', array()); ?>
    <main class="content">
        ...
        <div class="row justify-content-center" style="display: none" id="addSpecs">
            <div class="col-lg-12">
                <div class="card shadow-lg border-0 rounded-lg mt-5">
                    <div class="card-header"><h3 class="text-center font-weight-light my-4">Aggiungi Qualifica</h3></div>
                    <div class="card-body">
                        <form action="<?= base_url('admin/dashboard/formation/add') ?>" method="post">
                            <div class="form-floating mb-3">
                                <input name="userID" id="userID" type="hidden" value="">
                                <input name="spec" class="form-control" id="spec" type="text" placeholder="Specializzazione" />
                                <label for="spec">Specializzazione</label>
                            </div>
                            <div class="float-end">
                                <input type="submit" class="btn btn-primary center" id="addSpecs" value="Aggiungi">
                            </div>
        ...
        <div class="row justify-content-center" id="updateForm">
            <div class="col-lg-12">
                <div class="card shadow-lg border-0 rounded-lg mt-5">
                    <div class="card-header"><h3 class="text-center font-weight-light my-4">Storico Formazione</h3></div>
                    <div class="card-body">
                        <table class="table table-striped table-bordered" id="formazione">
                            <thead>
                            <tr>
                                <th>Qualifica</th>
                                <th>Aggiunta il</th>
    ...
    <script>
        ...
        const ac = new Autocomplete(field, {
            ...
            onSelectItem: ({label, value}) => {
                $.ajax({
                    url: "<?php echo(base_url('admin/dashboard/formation/getData')) ?>",
                    dataType: "json",
                    type: "Post",
                    async: true,
                    ...
                    success: function (data) {
                        $('#addSpecs').show();
                        $('#userID').val(data[0].user);
                        $('#formazione tbody').empty();
                        $.each(data, function( index, value ) {
                            $('#formazione tbody').append(
                                '<tr><td>' + data[index].qualifica + '</td><td>' + data[index].created_at + '</td></tr>'
                            );
                        });
                    },
